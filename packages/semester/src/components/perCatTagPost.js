...
import { getPostsGroupedByCategory, getPostsGroupedByCategoryAndTag } from "./helper";    
 ...
 */
const PerCatTagPost = ({ state, actions, libraries ,tagId}) => {
  // Get information about the current URL.
  const data = state.source.get(state.router.link);
  const postsPerCategory = (tagId?getPostsGroupedByCategoryAndTag(state.source,tagId):getPostsGroupedByCategory(state.source));
  const resultEvent = postsPerCategory.filter(Categ => Categ.category.link == "/category/events/");
  const resultFact = postsPerCategory.filter(Categ => (Categ.category.link == "/category/culture/" | Categ.category.link == "/category/lifestyle/" | Categ.category.link == "/category/initiative/" | Categ.category.link == "/category/science/" ));
  //const result = (state.router.link=='/' | state.router.link=='/regionofthemonth') ? postsPerCategory : (state.route.link=='/category/events')? resultEvent : resultFact;
...
export default connect(PerCatTagPost);
